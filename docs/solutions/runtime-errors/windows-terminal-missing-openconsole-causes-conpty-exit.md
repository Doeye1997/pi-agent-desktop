...
The preparation script now includes `OpenConsole.exe` in the completeness check ([`prepare-windows-terminal-host.mjs:46`](../../../scripts/prepare-windows-terminal-host.mjs)) and copies it from the same Windows Terminal build root as the terminal DLLs ([`prepare-windows-terminal-host.mjs:150`](../../../scripts/prepare-windows-terminal-host.mjs)). A staging directory missing either file is rebuilt instead of being accepted as complete.

The native host also uses one generated session GUID for both control settings and connection initialization ([`pi-session-display-host.cpp:694`](../../../native/wt-xaml-island/pi-session-display-host.cpp), [`pi-session-display-host.cpp:846`](../../../native/wt-xaml-island/pi-session-display-host.cpp)). When the connection closes, it marks the mounted display dead without deleting the native session, allowing a `RestartTerminalRequested` event to reconnect it ([`pi-session-display-host.cpp:912`](../../../native/wt-xaml-island/pi-session-display-host.cpp), [`pi-session-display-host.cpp:947`](../../../native/wt-xaml-island/pi-session-display-host.cpp)). The TypeScript manager preserves that mount on a dead mark and continues forwarding input; its regression test sends Enter after the dead mark ([`session-display.test.mjs:107`](../../../src/main/fork/session-display.test.mjs)).

Finally, the host releases its own references but keeps the manually loaded WinRT/XAML modules loaded for the process lifetime, leaving final unload ordering to the Windows loader ([`pi-session-display-host.cpp:316`](../../../native/wt-xaml-island/pi-session-display-host.cpp), [`pi-session-display-host.cpp:642`](../../../native/wt-xaml-island/pi-session-display-host.cpp)).
...
- Keep `OpenConsole.exe` in both the copy manifest and the staging-completeness predicate. The regression test enforces both requirements ([`windows-terminal-host.test.mjs:23`](../../../src/main/fork/windows-terminal-host.test.mjs)).
